...
void VideoContext::draw(const Coord &camera, const TestObject &obj) noexcept {
  ...
  dest.w /= 2;
  dest.h /= 2;
  dest.x = static_cast<int>(obj.getPosition().x + camera.x);
  dest.y = static_cast<int>(obj.getPosition().y + camera.y);
  ...
  dest.x = static_cast<int>(obj.getPosition().x - 40 + camera.x);
  dest.y = static_cast<int>(obj.getPosition().y + camera.y);
  dest.w /= 2;
  dest.h /= 2;
